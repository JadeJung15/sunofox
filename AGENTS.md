...
- UI 작업은 Astro + Tailwind + 기존 `src/styles/global.css` 커스텀 클래스 구조를 유지하세요. daisyUI는 설치 대상이 아니라 버튼, 카드, 배지, 탭, 모달 같은 의미 체계 참고용으로만 사용하세요.
- shadcn/ui와 HeroUI는 React 컴포넌트 도입 없이 레이아웃 밀도, 상태 표현, 접근성 패턴, hover/focus/loading 같은 상호작용 참고용으로만 사용하세요. 명시 승인 없이 React, Radix, HeroUI, Framer Motion, daisyUI 의존성을 추가하지 마세요.
- 공개 홈페이지에는 Bootstrap, Bulma, Chakra UI, Ant Design 스타일을 섞지 마세요. 관리자/폼/테이블 중심 화면에서는 Mantine 또는 Ant Design의 정보 구조 패턴만 참고할 수 있지만, 의존성 추가나 전체 톤 변경은 별도 승인 전에는 하지 마세요.
- SunoFox 공개 UI 톤은 Anime OST archive, 음악/세계관 브랜드 사이트 느낌을 유지하고, 색상/간격/버튼/카드/상태 표현은 기존 CSS 토큰과 클래스 체계를 우선하세요.
