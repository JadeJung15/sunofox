...
const assetVersion = '20260713-console-dark';
...
const darkSurfaceIndex = css.indexOf('/* Creator Console dark surface completion. */');
assert.ok(darkSurfaceIndex > css.lastIndexOf('background: #ffffff;'), 'Creator Console dark surface completion must follow every legacy white surface rule');
const darkSurfaceCss = css.slice(darkSurfaceIndex);
for (const selector of [
  '.mv-home-heading .mv-page-kicker',
  '.mv-import-hero',
  '.mv-import-studio .mv-workflow-card',
  '.mv-help-studio .mv-workflow-card',
  '.mv-storyboard-view .mv-hero',
  '.mv-storyboard-view .mv-panel',
  '.mv-console-workspace .mv-assist-panel',
  '.mv-console-workspace .mv-file-panel'
]) assert.ok(darkSurfaceCss.includes(selector), `dark surface completion must cover ${selector}`);
for (const marker of ['background: var(--mv-console-panel);', 'background: var(--mv-console-raised);', 'border-color: var(--mv-console-border);', 'color: var(--mv-console-text);', 'color: var(--mv-console-muted);']) assert.ok(darkSurfaceCss.includes(marker), `dark surface completion missing token: ${marker}`);
