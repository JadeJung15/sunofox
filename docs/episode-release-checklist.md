...
`check:content`는 `novelContent.js`의 회차 데이터와 각 에피소드 frontmatter의 제목, canonical, 공개일, ISO 날짜, 읽는 시간, 이전/다음 링크, 본문 존재 여부, 공유 태그 누락 여부를 비교합니다. 또한 `novelProject.readingPath`가 공개 회차 전체를 빠짐없이 포함하고 각 구간의 첫 화로 연결되는지 확인합니다.
