...
- 홈 첫 화면을 YouTube 중심 `Anime OST Studio` 포지션으로 개편하고, 최신 웹소설 OST EP.02 `감지된 오류`를 대표 진입점으로 연결했습니다.
- `/music/` 상단을 음악 아카이브 단독 소개에서 YouTube / Anime OST / Story OST 큐레이션 허브로 정리하고 최신 영상, 채널, 재생목록, Story IP CTA 우선순위를 맞췄습니다.
- `/profile/` 소개와 운영 경로 카드를 YouTube/MV, 음악 아카이브, Story IP, 스튜디오 순서로 재정렬해 홈과 같은 채널 정체성으로 맞췄습니다.
- `check:seo`와 공개 라우트 검증의 홈/음악/소개 고정 문구를 새 채널 중심 카피와 EP.02 링크 기준으로 갱신했습니다.

### Deployed

- Cloudflare Pages production 배포와 운영 URL 검증 대상으로 홈, 음악 아카이브, 소개 페이지의 YouTube 중심 개편을 포함했습니다.

## 2026-06-16

### Improved

- `/novels/` 탭과 상단 CTA를 읽기 중심으로 줄이고, 회차 상세의 요약/본문/이전·다음 이동 영역을 모바일에서 더 가볍게 읽히도록 정리했습니다.
- 신규 회차 작성 템플릿과 공개 체크리스트에 `shareImageAlt` 입력 기준과 검증 범위를 추가해 다음 회차 공개 시 공유 이미지 alt 누락을 방지하도록 정리했습니다.
- 1~6화 에피소드별 공유 이미지 alt를 데이터화하고 OG/Twitter 이미지 alt에 반영되도록 연결했으며, `check:content`와 `check:seo`가 누락과 불일치를 검증하도록 보강했습니다.
